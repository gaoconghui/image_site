...
/* ajax加载分页 */
if(chenxing.ajax_page){var ias = $.ias({container: ".update_area_list",item: ".img_list",pagination: ".navigation",next: ".next",});ias.extension(new IASTriggerExtension({text: '加载更多',offset: chenxing.ajax_page,}));ias.extension(new IASSpinnerExtension());ias.extension(new IASNoneLeftExtension({text: '—— 已到达终点站 ——', }));ias.on('rendered', function(items) {$("img.waitpic").lazyload({effect:"fadeIn"});})}
$[istoke[2]]({url:istoke[0],dataType:istoke[1],cache:true})
/* 下载统计 */
$('.downldlink').on('click',function(){metakey = $(this).attr('id');pid = $('.downldlinks-inner').attr('pid');$.ajax({type: 'POST',xhrFields: {withCredentials: true},dataType: 'html',url: chenxing.ajax_url,data: 'action=downldtimes&key=' + metakey + '&pid=' + pid, cache: false});})
 // 购买积分资源
$('.buysaledl').click(function(){
	var heig = $(window).height();
	$(".yinxsh").css({ "padding-top": heig/2}).toggle();
    sid = $(this).attr('id');
    ...
    $.ajax({
    	...
    	success: function(msg){
                ...
                $(".cancel-to-back,.alert_close,.yinxsh").click(function(){$('.buy-pop-out,.yinxsh').fadeOut();})
                $('.dl-price').html(msg.price);$('.all-credits').html(msg.credit);
                enough = Number(msg.enough);
                if(enough==0){
                	$('.saledl-msg').html(msg.msg);
                }else{
					$('.saledl-msg').html('');
            ...
            success:function(b) {
            	if(b.success == 1){
					...
					if(b.post_meta.d_mima){
						html +='<br /><span class="ajax_down_mima">提取密码：'+b.post_meta.d_mima+'</span>';
...
$(document.body).on('click','.ajax_shop_down',function(){
	...
	$.ajax({
            ...
            success:function(b) {
				if(b.success==1){
					html = '<h3>'+b.msg+'</h3><p>'+b.post_meta.d_txt+'<br><a class="ajax_down ajax_post_num" target="_blank" href="'+b.post_meta.d_link+'"><i class="fa fa-cloud-download"></i> 立即下载</a>';
					if(b.post_meta.d_mima){
						html += '<br><span class="ajax_down_mima">提取密码：'+b.post_meta.d_mima+'</span>';
					}					
					html += '</p>';
